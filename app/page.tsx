    ...
    return (
      <div className="px-4 pt-6 pb-28 max-w-md mx-auto w-full">
        <div className="flex items-center justify-between mb-6">
          <div className="bg-surface rounded-lg h-8 w-32 animate-pulse" />
          <div className="bg-surface rounded-lg h-8 w-8 animate-pulse" />
        </div>
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="bg-surface rounded-2xl h-24 mb-3 animate-pulse border border-bdf" />
        ))}
      </div>
    )
  }
    ...
    <div className="px-4 pt-6 pb-28 max-w-md mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-tx">{settings.business_name}</h1>
            {syncing && (
              <div className="w-1.5 h-1.5 bg-bda rounded-full animate-pulse" />
            )}
          </div>
          <p className="text-sm text-t4">{monthly.monthName}</p>
        </div>
        <Link
          href="/settings"
          className="w-10 h-10 flex items-center justify-center text-t5 hover:text-tx rounded-xl transition-colors"
        >
      ...
      {monthly.latestProject && (
        <>
          ...
          <Link href="/projects" className="block">
            <div className="bg-surface border border-bdr rounded-2xl p-4 mb-5 active:border-ac transition-colors">
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-bold text-tx">{monthly.latestProject.name}</p>
                  <p className="text-2xl font-bold mt-1 text-tx">{formatINR(monthly.latestProject.amount)}</p>
                  <p className="text-xs text-t4 mt-1">{monthly.latestProject.date}</p>
                </div>
                <span className={`text-xs px-3 py-1.5 rounded-full font-bold uppercase tracking-wide ${
                  monthly.latestProject.status === 'Paid'      ? 'bg-ac text-acf' :
                  monthly.latestProject.status === 'Confirmed' ? 'border border-ac text-tx' :
                  'border border-bda text-t3'
                }`}>
      ...
      <div className="bg-surface border border-bdr rounded-2xl p-4 mb-5">
        <div className="flex justify-between items-end mb-3">
          <div>
            <p className="text-xs text-t4 mb-0.5">Your margin</p>
            <p className="text-3xl font-bold text-tx">
              {monthly.margin < 0 ? '−' : ''}{Math.abs(monthly.margin).toFixed(1)}%
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-t4 mb-0.5">Industry avg</p>
            <p className="text-3xl font-bold text-tx">25%</p>
          </div>
        </div>

        <div className="h-2.5 bg-bdf rounded-full overflow-hidden mb-1">
          <div
            className="h-full bg-ac rounded-full"
            style={{ width: `${Math.max(2, Math.min(((monthly.margin + 30) / 55) * 100, 100))}%` }}
          />
        </div>
        <div className="flex justify-between text-xs text-t5 mb-3">
          <span>−30%</span>
          <span>25% target</span>
        </div>
        ...
        <div className="mt-4 pt-3 border-t border-bdf">
          ...
          <div className="h-2 bg-bdf rounded-full overflow-hidden">
            <div
              className="h-full bg-ac rounded-full"
              style={{ width: `${Math.min((monthly.revenue / 25000) * 100, 100)}%` }}
            />
          </div>
          <p className="text-xs text-t5 mt-1">
            {monthly.revenue >= 25000 ? 'Milestone cleared' : `${formatINR(25000 - monthly.revenue)} away`}
          </p>
        </div>

        <div className="mt-3 pt-3 border-t border-bdf flex justify-between text-sm">
          <span className="text-t4">Active projects</span>
          <span className="font-bold text-tx">
            {monthly.activeProjectCount}{' '}
            {monthly.activeProjectCount >= 2 ? '— ahead of avg' : '— build pipeline'}
          </span>
        </div>
      </div>
        ...
        {monthly.insights.map((ins, i) => (
          <div
            ...
            className={`rounded-2xl px-4 py-3 border flex items-center justify-between gap-3 ${
              ins.positive ? 'border-ac/20 bg-ac/5' : 'border-bdr bg-surface'
            }`}
          >
            <p className="text-sm text-t2 leading-snug flex-1">{ins.label}</p>
...
function Label({ children }: { children: React.ReactNode }) {
  return <p className="text-xs uppercase tracking-widest text-t5 mb-3">{children}</p>
}

function Card({ label, children, alert }: { label: string; children: React.ReactNode; alert?: boolean }) {
  return (
    <div className={`bg-surface rounded-2xl p-4 border ${alert ? 'border-ac' : 'border-bdr'}`}>
      <p className="text-xs uppercase tracking-widest text-t4 mb-2">{label}</p>
      {children}
    </div>
  )
}
...
function MiniCard({ label, value, alert }: { label: string; value: string; alert?: boolean }) {
  return (
    <div className={`bg-surface rounded-xl p-3 border text-center ${alert ? 'border-ac' : 'border-bdr'}`}>
      <p className="text-[10px] uppercase tracking-widest text-t4 mb-1">{label}</p>
      <p className="text-sm font-bold leading-tight text-tx">{value}</p>
    </div>
  )
}
